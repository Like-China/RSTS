...
def evaluate(src, model, max_length):
    ...
    input = torch.LongTensor([[settings.BOS]])
    trg = []
...
def get_predict_interaction(args):
    """
...
def get_predict_from_file(args):
    """
    读取trj.t中的轨迹，返回最后一层为 向量表示
    :param args:
    :return:
    """
    m0 = EncoderDecoder(args.vocab_size, args.embedding_size,

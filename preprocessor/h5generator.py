...
class Processor:
    ...
    def go(self):
        ...
        for ii in tqdm(range(self.read_txt_size//self.each_read)):
            # 读取一批轨迹，单个item存储为[[time, lon, lat],[time,lon,lat],]
    ...
    def read_files(self):
        ...
        for file in all_file_list:
            ...
            if (len(single_data)) < 100:
                continue  # 如果该 taxi文本记录的轨迹信息太少，则不记录该轨迹信息
            # 将满足条件的轨迹按照['id', 'times', 'longitude', 'latitude'] 存入all_data中
        ...
        while i < len(location):
            ...
            # 将时间在同一天的 划分为一条轨迹，位置点序列存储到single_taj_with_time中，只存储时间点+经纬度，不记录准确的天数

...
"""
读取一定数量taxi文本， 生成满足条件的轨迹文本 h5
porto已经有对应生成的h5文件，主要是对beijing taxi数据集生成对等格式的h5文件
"""
...
class Processor:
    ...
    def __init__(self, read_txt_size, each_read):
        ...
        # taxi 文本文件路径
        self.file_dir = os.path.join("E://data/taxi_log_2008_by_id")
        # 当前写了多少个trips到h5文件中
        self.valid_trip_nums = 0
        # 存储h5文件的路径
        self.h5_filepath = os.path.join("E://data/beijing.h5")
        # 用于过滤采样间隔较小的轨迹, 两个采样点间时间小于10, 我们认为采样过于频繁，应当舍去
    ...
    def address_data(self, batch_trjs):
        """
        处理一批轨迹，如果轨迹太长，则划分轨迹
        :param batch_trjs: 要处理的一批轨迹
        :return: self.tajs_with_ts 记录location+时间戳， self.tajs_with_time 记录location+具体时间
        """
        # [经纬度坐标，时间]
        self.tajs_with_time = []
        # [经纬度坐标, 转化的时间戳]
    ...
    def output_src_tgr(self, f):
        ...
        for each_taj in self.tajs_with_ts:
            # 再一次控制轨迹长度
            if self.min_length<=len(each_taj)<=self.max_length or len(each_taj):

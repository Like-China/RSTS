...
class EDwP:
    ...
    def insert(self,T1,T2,time_weight):
        ...
        if flag == 0:
            ...
            rep_cost = np.linalg.norm(np.array(a1[0:2])-np.array(b1[0:2]))+\
                np.linalg.norm(np.array(a2[0:2])-np.array(b2[0:2]))+\
                time_weight*(a1[2]-b1[2]+a2[2]-b2[2])/(86400*86400)
            T1.remove(T1[0])
        else:
            ''' 计算插入点在 T1 e1上占的长度比例'''
            len_e1 = np.linalg.norm(np.array(a1[0:2])-np.array(a2[0:2]))
            ...
            rep_cost = np.linalg.norm(np.array(a1[0:2])-np.array(b1[0:2]))+\
                np.linalg.norm(np.array(p[0:2])-np.array(b2[0:2]))+\
                    time_weight*(a1[2]-b1[2]+p[2]-b2[2])/(86400*86400)
    ...
    ''' 动态规划求解 EDwP'''
    def dp_EDwP(self, T1, T2, time_weight):
        ''' 计算T1和T2的长总度'''
        ...
        while(True):
            if(len(T1) <=1):
                ...
                for ii in range(len(T2)-1):
                    ...
                    rep_cost = np.linalg.norm(np.array(a1[0:2])-np.array(b1[0:2]))+\
                        ...
                        +time_weight*(a1[2]-b1[2]+a1[2]-b2[2])/(86400*86400)
                    all_converage += len_e
            ...
            if(a2[2] < b2[2]):
                conv,cost = self.insert(T2, T1,time_weight)
                # T1.remove(T1[0])
                T1 = T1[1:]
            else:
                conv,cost = self.insert(T1, T2,time_weight)
                # T2.remove(T2[0])
                T2 = T2[1:]
            all_converage += conv
            all_cost += cost
...
def func(T1, T2, costs, index, num_T1, num_T2, Processes_num):
    ...
    for i in range(begin, end):
        ...
        for j in range(0, num_T2):
            cost[j] = b2.dp_EDwP(copy.deepcopy(T1[i]), copy.deepcopy(T2[j]),p.time_w)
        costs[i] = cost
        # costs.append(cost)
        # print(cost)
        
def EDwP_multi_process(T1, T2, Processes_num):
     # b2 = EDwP()
     manger = Manager()
...
def func1(T1, T2, costs, index, num_T1, num_T2, Processes_num):
    ...
    b2 = EDwP()
    for i in range(begin, end):
        costs[i] = b2.dp_EDwP(copy.deepcopy(T1[i]), copy.deepcopy(T2[i]),p.time_w)
        
def EDwP_multi_process1(T1, T2, Processes_num):
     # b2 = EDwP()
     manger = Manager()
     costs = manger.dict()  # 使用字典
     time1 = time.time()
...
if __name__ == "__main__":
    ...
    t1 = [[116.42008999999999, 39.96032, 441.0], [116.41283, 39.95265, 1410.0], [116.47835, 39.913959999999996, 4019.0],
          [116.47185, 39.90733, 4320.0], [116.45806999999999, 39.94815, 5701.0], [116.47018, 39.95265, 6225.0],
          [116.48226000000001, 39.97217, 6828.0], [116.46824, 39.94185, 7432.0], [116.46875, 39.93646, 8337.0],
          [116.46021, 39.95841, 8941.0], [116.48308999999999, 39.98877, 9544.0],
          [116.50894, 40.012640000000005, 35279.0], [116.4866, 40.00656, 35743.0], [116.4653, 40.00062, 37695.0],
          [116.45433, 39.98806, 38020.0], [116.43418999999999, 39.926559999999995, 40969.0],
          [116.43675, 39.93508, 41573.0], [116.43845, 39.93934, 42180.0], [116.42055, 39.904509999999995, 43377.0],
          [116.47273, 39.92695, 45164.0], [116.44698000000001, 39.922109999999996, 45767.0],
          [116.41618999999999, 39.92305, 46371.0], [116.39283999999999, 39.922, 46673.0],
          [116.37024, 39.908609999999996, 47276.0], [116.30365, 39.9046, 47880.0]]
    

    t2 = [[116.4226, 39.96092, 743.0], [116.4292, 39.88944, 2958.0], [116.47185, 39.90733, 4320.0],
          [116.49298999999999, 39.95841, 5144.0], [116.46586, 39.948479999999996, 5923.0],
          [116.48383999999999, 39.96839, 6526.0], [116.47873999999999, 39.95558, 7130.0],
          [116.46866000000001, 39.9364, 8035.0], [116.46841, 39.94123, 8639.0], [116.4777, 39.97484, 9242.0],
          [116.5157, 40.021679999999996, 9846.0], [116.48811, 40.00525, 35581.0], [116.486, 40.0067, 37393.0],
          [116.45575, 39.98833, 37997.0], [116.43233000000001, 39.98082, 38322.0], [116.43055, 39.93002, 41271.0],
          [116.44253, 39.9369, 41869.0], [116.42823999999999, 39.94085, 42482.0],
          [116.47623999999999, 39.926809999999996, 44890.0], [116.45629, 39.92198, 45466.0],
          [116.43348999999999, 39.922959999999996, 46069.0], [116.41618999999999, 39.92305, 46371.0],
          [116.37558, 39.91963, 46975.0], [116.33209, 39.90625, 47578.0], [116.30791, 39.91992, 48483.0]]
    T1 = []
    for i in range(num_T1//2):
        T1.append(copy.deepcopy(t1))
        T1.append(copy.deepcopy(t2))
    T2 = []
    for i in range(num_T2//2):
        T2.append(copy.deepcopy(t2))
        T2.append(copy.deepcopy(t1))

    Processes_num = 8
    
    costs = EDwP_multi_process1(T1, T2, Processes_num)
